...
d3.json("https://gbfs.citibikenyc.com/gbfs/en/station_information.json").then(infoRes => {

  // When the first API call completes, perform another call to the Citi Bike station status endpoint
  d3.json("https://gbfs.citibikenyc.com/gbfs/en/station_status.json").then(statusRes => {
    ...
    for (let i = 0; i < stationInfo.length; i++) {
      ...
      const station = { ...stationInfo[i], ...stationStatus[i] };

      // Determine the station status code
